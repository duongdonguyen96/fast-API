...
class CasaAccount(Base):
    ...
    acc_structrure_type_id = Column(ForeignKey('crm_acc_structure_type.acc_structure_type_id'), nullable=False,
...
t_crm_joint_acc_agree = Table(
    'crm_joint_acc_agree', metadata,
    Column('agreement_author_id', ForeignKey('crm_agreement_authorization.agreement_author_id'), primary_key=True,
           server_default=text("sys_guid() "), comment='Mã thỏa thuận - ủy quyền'),
    Column('joint_account_holder_id', ForeignKey('crm_joint_account_holder.joint_account_holder_id'),
           comment='Mã Thông tin đồng chủ sở hữu'),
    comment='Thỏa thuận/ ủy quyền đồng sở hữu'
)
