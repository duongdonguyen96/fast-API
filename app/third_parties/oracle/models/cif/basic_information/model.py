...
class Customer(Base):
    ...
    cif_number = Column('cif_num', VARCHAR(9), nullable=False, unique=True, comment='Mã khách hàng')
    ...
    mobile_number = Column('mobile_num', VARCHAR(10), nullable=False, comment='Số điện thoại di động')
    ...
    legal_agreement_flag = Column(NUMBER(1, 0, False), nullable=False, comment='Cờ thỏa thuận pháp lý (có/không)')
    advertising_marketing_flag = Column(NUMBER(1, 0, False), nullable=False,
                                        comment='Cờ nhận quảng cáo từ SCB ( có/không)')
    customer_relationship_flag = Column('cust_relationship_flag', NUMBER(1, 0, False), nullable=False,
    ...
    customer_economic_profession_id = Column('economic_profession_id',
                                             ...
                                             nullable=False, comment='Mã ngành nghề kinh tế')
    ...
    channel = relationship('CrmChannel')
    cust_category = relationship('CrmCustCategory')
    cust_classification = relationship('CrmCustClassification')
    cust_status = relationship('CrmCustomerStatu')
    cust_type = relationship('CrmCustType')
    economic_profession = relationship('CrmCustEconomicProfession')
    kyc_level = relationship('CrmKcyLevel')
    nationality = relationship('CrmAddressCountry')
    open_branch = relationship('CrmBranch')
