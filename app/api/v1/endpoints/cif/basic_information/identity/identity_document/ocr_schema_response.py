...
class FrontSideIdentityCitizenCardResponse(BaseSchema):
    ...
    identity_avatar_image_uuid: Optional[str] = Field(
        ..., min_length=1, description="Hình ảnh chân dung trên GTĐD dùng để so sánh với khuôn mặt"
    )
...
class OCRFrontSideBasicInfoIdentityCardResponse(BaseSchema):
    full_name_vn: Optional[str] = Field(..., min_length=1, description="Họ và tên", nullable=True)
...
class BackSideIdentityCardResponse(BaseSchema):
    identity_image_url: Optional[str] = Field(..., min_length=1, description="URL hình ảnh mặt sau CMND", nullable=True)
...
class OCRBackSideBasicInformationIdentityCardResponse(BaseSchema):
    ...
    identity_characteristic: Optional[str] = Field(..., min_length=1, description="Đặc điểm nhận dạng", nullable=True)
...
class OCRBasicInfoPassportResponse(BaseSchema):
    ...
    identity_card_number: Optional[str] = Field(..., min_length=1, description="Số CMND", nullable=True)
    mrz_content: Optional[str] = Field(None, min_length=1, description="Mã MRZ", nullable=True)
...
class OCRBackSideIdentityDocumentCitizenCardResponse(BaseSchema):
    issued_date: Optional[date] = Field(..., description="Ngày cấp", nullable=True)
    place_of_issue: Optional[DropdownResponse] = Field(..., description="Nơi cấp", nullable=True)
    mrz_content: Optional[str] = Field(..., min_length=1, description="Đặc điểm nhận dạng", nullable=True)
...
class CompareSuccessResponse(BaseSchema):
    ...
    face_uuid_ekyc: str = Field(..., description="uuid trả về từ hệ thống eKYC")
